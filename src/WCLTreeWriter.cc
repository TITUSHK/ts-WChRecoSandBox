...
WCLTreeWriter::WCLTreeWriter(TString filename, int mode)
{
  ...
  if(mode==2){
    fChain = new TTree("HitsTree","HitsTree");
    pmtTree = new TTree("PMTsTree","PMTsTree");
...
void WCLTreeWriter::AddPMT(int id, double x, double y, double z, double size, double qe, double tRes, int isLAPPD){
  pmt_id = id;
  pmt_pos_x = x;
  pmt_pos_y = y;
  pmt_pos_z = z;
...
void WCLTreeWriter::AddHit(double time, double x, double y, double z, int pmtId){
  hit_time[nhits] = time;
  hit_x[nhits] = x;
  hit_y[nhits] = y;
  hit_z[nhits] = z;
  hit_PMTid[nhits] = pmtId;
  nhits++;
}
...
void WCLTreeWriter::SetPMTBranchAddys()
{
  std::cout<<"Setting Branch Addresses"<<std::endl;

  const int knphotmax=10000000;

  hit_time = new Double_t[knphotmax];
  hit_x = new Double_t[knphotmax];
  hit_y = new Double_t[knphotmax];
  hit_z = new Double_t[knphotmax];
  hit_PMTid = new Int_t[knphotmax];

  fChain->Branch("evt",&eventcount);
  fChain->Branch("nhits",&nhits);
   
  fChain->Branch("hit_time",hit_time,"hit_time[nhits]/D");
  fChain->Branch("hit_x",hit_x,"hit_x[nhits]/D");
  fChain->Branch("hit_y",hit_y,"hit_y[nhits]/D");
  fChain->Branch("hit_z",hit_z,"hit_z[nhits]/D");
  fChain->Branch("hit_PMTid",hit_PMTid,"hit_PMTid[nhits]/I");
  ...
  pmtTree->Branch("pmt_id",&pmt_id);
  pmtTree->Branch("pmt_pos_x",&pmt_pos_x);
  pmtTree->Branch("pmt_pos_y",&pmt_pos_y);
  pmtTree->Branch("pmt_pos_z",&pmt_pos_z);
  pmtTree->Branch("pmt_size",&pmt_size);
  pmtTree->Branch("pmt_qe",&pmt_qe);
  pmtTree->Branch("pmt_time_res",&pmt_time_res);
  pmtTree->Branch("is_lappd",&is_lappd);
}
