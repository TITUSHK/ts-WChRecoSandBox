...
void SandBoxPMTcoverage::SetWallConfiguration(int mcase, int mmode, int mshapePMT, double msizePMT, double NrowPMT, double NcolPMT,
                                              double QE_PMT, double QE_WAVELENGTHS_PMT[20], double QE_FACTOR_PMT[20], int mshapeLAPPD,
                                              double msizeLAPPD, double NrowLAPPD, double NcolLAPPD, double QE_LAPPD)
{
  // order of cases : cylinder, xh, xl, yh, yl, zh, zl;
  for(int ii=0;ii<7;ii++){
    if(mcase==ii){
      _mmode[ii] = mmode;
      _QE_PMT[ii] = QE_PMT;
      for(int j=0; j<20; j++){
        _QE_WAVELENGTHS_PMT[ii][j] = QE_WAVELENGTHS_PMT[j];
        _QE_FACTOR_PMT[ii][j] = QE_FACTOR_PMT[j];
      }
...
int SandBoxPMTcoverage::isActiveHit(double mx, double my, double mz, double wl, int &whichPhotoSensor, bool LAPPDs){

  int wasithit=0;
  int wP=-5555;
  int wcase=-4;
  double mtheta = atan2(my,mx)*_Rh;
  if(fabs((mx*mx + my*my)-(_Rh*_Rh))<0.1 && _opt==0){ wcase=0; wasithit=this->isActiveHitC(wcase, mz, mtheta, wl, wP, LAPPDs); }
  else if(fabs(mx-_xh)<0.1 && _opt==1){ wcase=1; wasithit=this->isActiveHitC(wcase, my, mz, wl, wP, LAPPDs); }
  else if(fabs(mx-_xl)<0.1 && _opt==1){ wcase=2; wasithit=this->isActiveHitC(wcase, my, mz, wl, wP, LAPPDs); }
  else if(fabs(my-_yh)<0.1 && _opt==1){ wcase=3; wasithit=this->isActiveHitC(wcase, mx, mz, wl, wP, LAPPDs); }
  else if(fabs(my-_yl)<0.1 && _opt==1){ wcase=4; wasithit=this->isActiveHitC(wcase, mx, mz, wl, wP, LAPPDs); }
  else if(fabs(mz-_zh)<0.1){ wcase=5; wasithit=this->isActiveHitC(wcase, mx, my, wl, wP, LAPPDs); }
  else if(fabs(mz-_zl)<0.1){ wcase=6; wasithit=this->isActiveHitC(wcase, mx, my, wl, wP, LAPPDs); }
  //else std::cout << "Photon end point not on wall! (" << mx << ", " << my << ", " << mz << ")" << std::endl;
...
int SandBoxPMTcoverage::isActiveHitC(int mcase, double xx1, double xx2, double wl, int &whichPhotoSensor, bool LAPPDs){
  ...
  if(_mmode[mcase]==1){
    ...
    Double_t qe = 0;
    for(int i=0; i<19; i++) {
      if (wl < _QE_WAVELENGTHS_PMT[mcase][i + 1]) {
        double delta = (wl - _QE_WAVELENGTHS_PMT[mcase][i])/(_QE_WAVELENGTHS_PMT[mcase][i + 1] - _QE_WAVELENGTHS_PMT[mcase][i]);
        qe = _QE_PMT[mcase] * (_QE_FACTOR_PMT[mcase][i] * (1 - delta) + _QE_FACTOR_PMT[mcase][i + 1] * delta) / 100.;
        break;
      }
    }
    if(qe>0 && _PMTs_mshape[mcase]==1)
    {
      ...
      if( d1<(_PMTs_msize[mcase]/2) && d2<(_PMTs_msize[mcase]/2) 
			&& (d3<(_Rh*_Rh) || mcase==0 || _opt==1) && rd< qe){
	    washit=1;
	    PSnum=(int)(wxx1 + wxx2*100) + mcase*10000;
      }	
    }

    if(qe>0 && _PMTs_mshape[mcase]==0)
    {
      ...
      if( distance<(_PMTs_msize[mcase]/2) && (d3<_Rh || mcase==0 || _opt==1) && rd< qe){
	    washit=1;
	    PSnum=(int)(wxx1 + wxx2*100) + mcase*10000;
      }
    }
