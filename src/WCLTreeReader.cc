...
void WCLTreeReader::LoadData(const char* file)
{
  ...
  fChain->Add(file);
  if(fPMTs) {
    std::cout << "Adding file to PMT chain " << file << std::endl;
    pmtChain->Add(file);
    //Don't add PMTs for additional files (assume all use same PMTs)
    fPMTs = false;
  }
...
{
  std::cout << " *** WCLTreeReader::LoadData(...) *** " << std::endl;
  std::cout << "  adding: " << dfile << std::endl;

  fincgen=1;
  fChain->Add(dfile);
  std::cout  << " and " <<gfile<<std::endl;
  fChainT->Add(gfile);
  if(fPMTs)
  {
    std::cout << "Adding file to PMT chain " << dfile << std::endl;
    pmtChain->Add(dfile);
    //Don't add PMTs for additional files (assume all use same PMTs)
    fPMTs = false;
  }
...
void WCLTreeReader::LoadPMT(Int_t iPMT)
{
  pmtChain->GetEntry(iPMT);
}
...
Int_t WCLTreeReader::GetNpmts()
{
  return pmtChain->GetEntries();
}
...
WCLTreeReader::WCLTreeReader(int filetype, int includegen)
{
  ...
  else if(filetype==2) tname+="HitsTree";
  else tname+="EventTree";
 
  // create event fChain
  fChain = new TChain(tname,"fChain");
  if(includegen==1) fChainT = new TChain("tcardfile","fChain");
  ...
  if(filetype==2){
    fPMTs = true;
    pmtChain = new TChain("PMTsTree","PMTsTree");
    this->SetPMTBranchAddys();
  }
  else
  {
    fPMTs = false;
...
void WCLTreeReader::SetEventBranchAddys(int filetype)
{
  bool isSmeared=false;
  if(filetype!=0) isSmeared = true;
  ...
  */
  ...
  if(filetype!=2){
    ...
    const int knhitmax=1000000;

    hit_time = new double[knhitmax];
    hit_x = new double[knhitmax];
    hit_y = new double[knhitmax];
    hit_z = new double[knhitmax];
    hit_PMTid = new int[knhitmax];

    fChain->SetBranchAddress("evt",&eventcount);
     
    fChain->SetBranchAddress("hit_time",hit_time);
    fChain->SetBranchAddress("hit_x",hit_x);
    fChain->SetBranchAddress("hit_y",hit_y);
    fChain->SetBranchAddress("hit_z",hit_z);
    fChain->SetBranchAddress("hit_PMTid",hit_PMTid);
  }
  ...
  if(isSmeared){
    ...
    fChain->SetBranchAddress("evt",&mevt);
...
void WCLTreeReader::SetPMTBranchAddys(){

  std::cout<<"setting PMT branch addresses"<<std::endl;

  pmtChain->SetBranchAddress("pmt_id",&pmt_id);
  pmtChain->SetBranchAddress("pmt_pos_x",&pmt_pos_x);
  pmtChain->SetBranchAddress("pmt_pos_y",&pmt_pos_y);
  pmtChain->SetBranchAddress("pmt_pos_z",&pmt_pos_z);
  pmtChain->SetBranchAddress("pmt_size",&pmt_size);
  pmtChain->SetBranchAddress("pmt_qe",&pmt_qe);
  pmtChain->SetBranchAddress("pmt_time_res",&pmt_time_res);
  pmtChain->SetBranchAddress("is_lappd",&is_lappd);
}
