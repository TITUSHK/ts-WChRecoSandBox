...
//	cerr<<"ExpectedPMTPhotoelectrons"<<endl;        
...
//	cerr<<"ipnu----> "<<ipnu<<endl;
	...
	//Distance from vertex to PMT:
	double pmtDist = TMath::Sqrt(vtxToPMTx*vtxToPMTx+vtxToPMTy*vtxToPMTy+vtxToPMTz*vtxToPMTz);
	//double pmtDist = TMath::Max(  photonsLookup->GetXaxis()->GetBinCenter(1) +1e-4, TMath::Min( pmtDist,  photonsLookup->GetXaxis()->GetBinCenter(photonsLookup->GetNbinsX()) - 1e-4) );	
...
//	cerr<<"Error about to run."<<endl;
//cerr<<"pmtDist-------> "<< pmtDist<<endl;
///cerr<<"pmtTrackAngl--> "<<pmtTrackAngle <<endl;
//cerr<<"kineticEnergy-> "<< kineticEnergy<<endl;
...
//cerr<<"Exp.Phot-> "<<expectedPhotons<<endl;
//expectedPhotons = photonsLookup->Interpolate(pmtDist, pmtTrackAngle, kineticEnergy);//*100000000;//Remove this factor when likelihood tables normalisation is fixed
//cerr<<"Exp.Phot-> "<<expectedPhotons<<endl;
//	cerr<<"Error run."<<endl;
	if(expectedTime){
//                *expectedTime = timesLookup->GetBinContent(photonsLookup->FindBin(pmtDist, pmtTrackAngle, kineticEnergy));
		*expectedTime = timesLookup->Interpolate(pmtDist, pmtTrackAngle, kineticEnergy);
	//	cerr<<"this never runs"<<endl;
	}	
	
	//////////   static const double QE = 2.*0.22; 
