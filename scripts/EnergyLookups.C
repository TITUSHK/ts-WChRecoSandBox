...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      double toWallZ = ((recoDirZ[0]>0 ? 1100 : -1100)-recoVtxZ[0])/recoDirZ[0];
      double a = 1-recoDirZ[0]*recoDirZ[0];
      double b = recoVtxX[0]*recoDirX[0]+recoVtxY[0]*recoDirY[0];
      double c = recoVtxR2-550*550;
      double toWallR = (TMath::Sqrt(b*b-a*c)-b)/a;
      double toWall = toWallR < toWallZ ? toWallR : toWallZ;
      double vtxTrackBias = diffVtxX*recoDirX[0]+diffVtxY*recoDirY[0]+diffVtxZ*recoDirZ[0];
      nHitToWallKELookup_e->Fill(peak1nPEs[0], toWall, trueKE);
      nHitToWallVtxTrackBias_e->Fill(peak1nPEs[0], toWall, vtxTrackBias);
      nHitToWallCounts_e->Fill(peak1nPEs[0], toWall);
      nHitDWallKELookup_e->Fill(peak1nPEs[0], dWall, trueKE);
      nHitDWallVtxTrackBias_e->Fill(peak1nPEs[0], dWall, vtxTrackBias);
      nHitDWallCounts_e->Fill(peak1nPEs[0], dWall);
   }
   ...
   for (Long64_t jentry=0; jentry<nentries2;jentry++) {
      ...
      // if (Cut(ientry) < 0) continue;
      double recoVtxR2 = recoVtxX2[0]*recoVtxX2[0]+recoVtxY2[0]*recoVtxY2[0];
      double dWallR = 550-TMath::Sqrt(recoVtxR2);
      double dWallZ = 1100-TMath::Abs(recoVtxZ2[0]);
      double dWall = dWallR < dWallZ ? dWallR : dWallZ;
      double toWallZ = ((recoDirZ2[0]>0 ? 1100 : -1100)-recoVtxZ2[0])/recoDirZ2[0];
      double a = 1-recoDirZ2[0]*recoDirZ2[0];
      double b = recoVtxX2[0]*recoDirX2[0]+recoVtxY2[0]*recoDirY2[0];
      double c = recoVtxR2-550*550;
      double toWallR = (TMath::Sqrt(b*b-a*c)-b)/a;
      double toWall = toWallR < toWallZ ? toWallR : toWallZ;
      double vtxTrackBias = diffVtxX2*recoDirX2[0]+diffVtxY2*recoDirY2[0]+diffVtxZ2*recoDirZ2[0];
      nHitToWallKELookup_mu->Fill(peak1nPEs2[0], toWall, trueKE2);
      nHitToWallVtxTrackBias_mu->Fill(peak1nPEs2[0], toWall, vtxTrackBias);
      nHitToWallCounts_mu->Fill(peak1nPEs2[0], toWall);
      nHitDWallKELookup_mu->Fill(peak1nPEs2[0], dWall, trueKE2);
      nHitDWallVtxTrackBias_mu->Fill(peak1nPEs2[0], dWall, vtxTrackBias);
      nHitDWallCounts_mu->Fill(peak1nPEs2[0], dWall);
   }
