...
void PhotonMask_10inHQE(TString filename="../../FullEvent.root", TString genfilename="../../generatorcardfile.root",TString outfilename="out.root")
{
    ...
    for(int row = 0; row < NFront; row++){
      ...
      for(int col = 0; col < NFront; col++){
        ...
        if(TMath::Sqrt(pmtX*pmtX+pmtY*pmtY)+size/2<Rdet){
          ...
          mTW->AddPMT(nPMT,pmtX,pmtY,pmtZ,size,qe,tRes,0);
          pmtXs[nPMT]=pmtX;
          pmtYs[nPMT]=pmtY;
          pmtZs[nPMT]=pmtZ;
          pmtIDs[nPMT++]=pmtID;
  ...
  //add PMTs for cube option
    ...
    for(int row = 0; row < NbyN; row++){
      ...
      for(int col = 0; col < NbyN; col++){
        ...
        mTW->AddPMT(nPMT,Ldet,pmtRow,pmtCol,size,qe,tRes,0);
        pmtIDs[nPMT++]=pmtID+10000;
        mTW->AddPMT(nPMT,-Ldet,pmtRow,pmtCol,size,qe,tRes,0);
        pmtIDs[nPMT++]=pmtID+20000;
        mTW->AddPMT(nPMT,pmtRow,Ldet,pmtCol,size,qe,tRes,0);
        pmtIDs[nPMT++]=pmtID+30000;
        mTW->AddPMT(nPMT,pmtRow,-Ldet,pmtCol,size,qe,tRes,0);
        pmtIDs[nPMT++]=pmtID+40000;
        mTW->AddPMT(nPMT,pmtRow,pmtCol,Ldet,size,qe,tRes,0);
